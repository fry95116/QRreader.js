...
qrcode.decodeFromImageData = function (imageData, width, height) {
	qrcode.width = width;
	qrcode.height = height;
	qrcode.imagedata = imageData;  //这个是真正的数据源
	qrcode.result = qrcode.process(); //解析imagedata中的数据
	return qrcode.result
}

qrcode.decode = function (src) {

	if (arguments.length == 0) //如果没有参数传入
	{
		...
		} // 获取canvas对象和其context2D对象到canvas_qr与context中
		qrcode.width = canvas_qr.width;
		qrcode.height = canvas_qr.height;
		qrcode.imagedata = context.getImageData(0, 0, qrcode.width, qrcode.height);  //这个是真正的数据源
		qrcode.result = qrcode.process(context); //解析imagedata中的数据
...
qrcode.process = function (ctx) {
	...
	if (qrcode.debug) {
		...
		if (ctx) ctx.putImageData(qrcode.imagedata, 0, 0);
	}
